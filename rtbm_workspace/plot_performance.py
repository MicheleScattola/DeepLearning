...
def plot_scaling(rows, outdir, all_rows=None):
    ...
    to report the true attempted-seed count in the n=/warning text, since
    `rows` is the already status=='ok'-filtered set."""
    best_pb = best_pb_per_nh(rows)
    nh_values = sorted(best_pb)

    nll_mean, nll_std, auc_mean, auc_std, n_good_list = [], [], [], [], []
    for nh in nh_values:
        sel = [r for r in rows if r['nh'] == nh and r['param_bound'] == best_pb[nh]]
        n_attempted = (sum(1 for r in all_rows if r['nh'] == nh and r['param_bound'] == best_pb[nh])
        ...
        if n_crash or n_timeout_or_init_fail:
            print(f"  [WARN] N_h={int(nh)} (pb={best_pb[nh]:.2g}): of {n_attempted} attempted seed(s), "
        ...
        if len(good) < 2:
            print(f"  [WARN] N_h={int(nh)} (pb={best_pb[nh]:.2g}): only {len(good)} usable seed(s) "
    ...
    fig.suptitle('RTBM Scaling with $N_h$ (each at its best param\\_bound, '
                  'mean $\\pm$ std over non-crashed seeds)', fontweight='bold')
    plt.tight_layout()
    path = os.path.join(outdir, 'scaling_nh.png')
